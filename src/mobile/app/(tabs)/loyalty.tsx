    ...
    <SafeAreaView className="flex-1 bg-bg">
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingBottom: 32 }}>
        <View className="px-4 pt-4 pb-2">
          <Text className="text-2xl font-bold text-ink">Lojalność</Text>
        </View>

        <View className="mx-4 my-2 bg-accentSoft rounded-2xl p-6 items-center">
          <Text className="text-muted text-sm mb-2">Twoje saldo</Text>
          ...
          <Text className="text-muted text-sm mt-2">punktów</Text>
        </View>

        <Text className="text-muted px-4 mt-4 mb-2 text-sm uppercase">Historia</Text>
        {historyQuery.isPending ? (
          <ActivityIndicator className="mt-4" />
        ) : (historyQuery.data?.items ?? []).length === 0 ? (
          <Text className="text-muted text-center mt-8">Brak historii.</Text>
        ) : (
          <View className="px-4">
              ...
              <View key={item.id}>
                ...
                <View className="flex-row justify-between items-center py-2">
                  <View className="flex-1">
                    <Text className="text-ink">{item.reason ?? "(brak powodu)"}</Text>
