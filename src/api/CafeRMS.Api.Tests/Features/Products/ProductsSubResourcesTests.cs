...
public sealed class ProductsSubResourcesTests : IDisposable
{
    ...
    {
        var (companyId, productId, _) = await SeedProductWithDepsAsync();
        var tagId = await SeedTagAsync(companyId, "Vegan");
        using var client = factory.CreateClientAs(AccountType.Staff, companyId: companyId, permissions: [Permissions.ProductsManage]);
    ...
    {
        var (companyId, productId, _) = await SeedProductWithDepsAsync();
        var allergenId = await SeedAllergenAsync(companyId, "Peanuts");
        using var client = factory.CreateClientAs(AccountType.Staff, companyId: companyId, permissions: [Permissions.ProductsManage]);
    ...
    {
        var (companyId, productId, _) = await SeedProductWithDepsAsync();
        var groupId = await SeedModifierGroupAsync(companyId, "Milk type");
        using var client = factory.CreateClientAs(AccountType.Staff, companyId: companyId, permissions: [Permissions.ProductsManage]);
    ...
    {
        var (companyId, productId, _) = await SeedProductWithDepsAsync();
        var pgId = await SeedPriceGroupAsync(companyId, "Standard");
        using var client = factory.CreateClientAs(AccountType.Staff, companyId: companyId, permissions: [Permissions.ProductsManage]);
    ...
    private async Task<(Guid CompanyId, Guid ProductId, Guid TaxRateId)> SeedProductWithDepsAsync()
    {
        ...
        return (company.Id, product.Id, taxRate.Id);
    }
    ...
    private async Task<Guid> SeedModifierGroupAsync(Guid companyId, string name)
    {
        using var scope = factory.Services.CreateScope();
        var db = scope.ServiceProvider.GetRequiredService<AppDbContext>();
