...
import { printoutTemplatesApi } from "./api"

const PAGE_SIZE = 20

export function PrintoutTemplatesTable() {
  ...
  const query = useQuery({
    queryKey: ["printout-templates", { page, filter }],
    queryFn: () => printoutTemplatesApi.list({ page, pageSize: PAGE_SIZE, name: filter || undefined, sort: "name" }),
  })

  const deleteMutation = useMutation({
    mutationFn: (id: string) => printoutTemplatesApi.remove(id),
    onSuccess: () => {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <Button render={<Link href="/admin/printout-templates/new" />} nativeButton={false}>
          ...
          <TableBody>
              ...
              <TableRow>
                <TableCell colSpan={4} className="py-12">
                  <div className="flex flex-col items-center gap-3 text-muted-foreground">
                    ...
                    <p className="text-sm">No printout templates yet.</p>
                  </div>
                </TableCell>
              </TableRow>
            )}
            {query.data?.items.map((t) => (
              <TableRow key={t.id}>
                ...
                <TableCell className="text-sm font-mono text-muted-foreground truncate max-w-md">{t.templateFileUrl}</TableCell>
                ...
                <TableCell>
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem render={<Link href={`/admin/printout-templates/${t.id}`} />}>
                        <Pencil className="mr-2 h-4 w-4" />Edit
                      </DropdownMenuItem>
                      <DropdownMenuItem className="text-destructive focus:text-destructive" onClick={() => setConfirmDelete({ id: t.id, name: t.name })}>
