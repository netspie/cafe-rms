...
import { modifierGroupsApi } from "./api"
...
export function ModifierGroupsTable() {
  ...
  const query = useQuery({
    queryKey: ["modifier-groups", { page, filter }],
    queryFn: () => modifierGroupsApi.list({ page, pageSize: PAGE_SIZE, name: filter || undefined, sort: "name" }),
  })
  ...
  const deleteMutation = useMutation({
    ...
    onError: (err) => {
      ...
      else toast.error("Could not delete modifier group")
    },
  })
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <Button render={<Link href="/admin/modifier-groups/new" />} nativeButton={false}>
                ...
                <TableCell>
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem render={<Link href={`/admin/modifier-groups/${g.id}`} />}>
                        <Pencil className="mr-2 h-4 w-4" />Edit
                      </DropdownMenuItem>
                      <DropdownMenuItem className="text-destructive focus:text-destructive" onClick={() => setConfirmDelete({ id: g.id, name: g.name })}>
      ...
      <Dialog open={!!confirmDelete} onOpenChange={(open) => !open && setConfirmDelete(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Removes &ldquo;{confirmDelete?.name}&rdquo;. Modifiers in this group must be moved or deleted first.</DialogDescription>
          </DialogHeader>
          <DialogFooter>
