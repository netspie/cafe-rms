...
export default function ProductListsListPage() {
  ...
  const [confirmDelete, setConfirmDelete] = useState<ProductListItem | null>(null)

  useEffect(() => {
    setLoading(true)
    const params = new URLSearchParams({ page: String(page), pageSize: String(PAGE_SIZE), sort: "name" })
    if (filter) params.set("name", filter)
    api.get<PagedResult<ProductListItem>>(`/api/product-lists?${params}`)
      .then(setData)
  ...
  async function handleDelete(item: ProductListItem) {
    try {
      await api.delete(`/api/product-lists/${item.id}`)
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <button onClick={() => setShowAdd(true)} className="inline-flex h-9 items-center gap-2 rounded-md bg-primary px-3 text-sm font-medium text-primary-foreground hover:opacity-90">
          <Plus className="h-4 w-4" />New product list
        </button>
      </div>
              ...
              <tr key={it.id} className="border-t">
                ...
                <td className="px-3 py-2">
                  <div className="flex justify-end gap-1">
                    <Link href={`/admin/product-lists/${it.id}`} className="inline-flex h-8 w-8 items-center justify-center rounded-md hover:bg-accent" aria-label="Edit"><Pencil className="h-4 w-4" /></Link>
      ...
      {confirmDelete && <ConfirmDialog title="Delete product list?" description={`Removes "${confirmDelete.name}". Events using it fall back to default pricing.`} onConfirm={() => handleDelete(confirmDelete)} onCancel={() => setConfirmDelete(null)} />}
    </div>
  )
}
...
function AddDialog({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setErrors({})
    try {
      await api.post("/api/product-lists", { name })
  ...
  return (
    <Modal title="New product list" onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-4">
        <Field label="Name" error={errors.name}>
          <input value={name} onChange={(e) => setName(e.target.value)} required placeholder="Brunch menu / Vegan corner" className="h-9 w-full rounded-md border bg-transparent px-3 text-sm outline-none focus:border-ring focus:ring-2 focus:ring-ring/30" />
        </Field>
        <div className="flex justify-end gap-2">
          <button type="button" onClick={onClose} className="h-9 rounded-md px-3 text-sm hover:bg-accent">Cancel</button>
          <button type="submit" disabled={submitting} className="h-9 rounded-md bg-primary px-3 text-sm font-medium text-primary-foreground hover:opacity-90 disabled:opacity-50">{submitting ? "Saving…" : "Create product list"}</button>
        </div>
      </form>
    </Modal>
  )
