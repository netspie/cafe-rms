...
async function createProductList(formData: FormData) {
  "use server"
  await api.post("/api/product-lists", { name: formData.get("name") as string })
  revalidatePath("/admin/product-lists")
}

async function deleteProductList(id: string) {
  "use server"
  await api.delete(`/api/product-lists/${id}`)
  revalidatePath("/admin/product-lists")
}

export default async function ProductListsListPage({ searchParams }: { searchParams: Promise<{ page?: string; q?: string }> }) {
  ...
  const data = await api.get<PagedResult<ProductListItem>>(`/api/product-lists?${search}`)
  ...
  return (
    <div className="space-y-6">
      ...
      <form action={createProductList} className="flex items-end gap-3 rounded-md border bg-card p-3">
        <Field label="Name">
          <input name="name" required placeholder="Brunch menu / Vegan corner" className="h-9 w-72 rounded-md border bg-transparent px-3 text-sm outline-none focus:border-ring focus:ring-2 focus:ring-ring/30" />
        </Field>
            ...
            {data.items.length === 0 && (
              <tr><td colSpan={3} className="px-3 py-12">
                <div className="flex flex-col items-center gap-3 text-muted-foreground">
                  ...
                  <p>No product lists match this filter.</p>
                </div>
              </td></tr>
            )}
            {data.items.map((it) => (
              <tr key={it.id} className="border-t">
                ...
                <td className="px-3 py-2">
                  <div className="flex justify-end gap-1">
                    ...
                    <form action={deleteProductList.bind(null, it.id)}>
