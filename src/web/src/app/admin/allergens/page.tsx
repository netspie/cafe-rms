...
async function createAllergen(formData: FormData) {
  "use server"
  await api.post("/api/allergens", { name: formData.get("name") as string })
...
export default async function AllergensListPage({ searchParams }: { searchParams: Promise<{ page?: string; q?: string; sort?: string }> }) {
  ...
  const data = await api.get<PagedResult<AllergenItem>>(`/api/allergens?${search}`)
  ...
  return (
    <div className="space-y-6">
      ...
      <form action={createAllergen} className="flex items-end gap-3 rounded-md border bg-card p-3">
        <Field label="Name">
          <input name="name" required placeholder="New allergen" className="h-9 w-48 rounded-md border bg-transparent px-3 text-sm outline-none focus:border-ring focus:ring-2 focus:ring-ring/30" />
        </Field>
            ...
            {data.items.length === 0 && (
              <tr><td colSpan={3} className="px-3 py-12">
                <div className="flex flex-col items-center gap-3 text-muted-foreground">
                  ...
                  <p>No allergens match this filter.</p>
                </div>
              </td></tr>
            )}
            {data.items.map((it) => (
              <tr key={it.id} className="border-t">
                ...
                <td className="px-3 py-2">
                  <div className="flex justify-end gap-1">
                    ...
                    <form action={deleteAllergen.bind(null, it.id)}>
