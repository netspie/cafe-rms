...
export default function ModifiersListPage() {
  ...
  const [data, setData] = useState<PagedResult<ModifierItem> | null>(null)
  const [groups, setGroups] = useState<ModifierGroupRow[]>([])
  const [loading, setLoading] = useState(true)
  const [refreshKey, setRefreshKey] = useState(0)
  const [showAdd, setShowAdd] = useState(false)
  const [confirmDelete, setConfirmDelete] = useState<ModifierItem | null>(null)

  useEffect(() => {
    api.get<PagedResult<ModifierGroupRow>>(`/api/modifier-groups?page=1&pageSize=100&sort=name`)
  ...
  useEffect(() => {
    ...
    if (groupId) params.set("modifierGroupId", groupId)
    api.get<PagedResult<ModifierItem>>(`/api/modifiers?${params}`)
      .then(setData)
      .catch((err) => { if (err instanceof ApiError) toast.error(err.message) })
      .finally(() => setLoading(false))
  }, [page, filter, groupId, refreshKey])

  async function handleDelete(item: ModifierItem) {
    try {
      await api.delete(`/api/modifiers/${item.id}`)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <select value={groupId} onChange={(e) => { setPage(1); setGroupId(e.target.value) }} className="h-9 max-w-xs rounded-md border bg-transparent px-3 text-sm outline-none focus:border-ring focus:ring-2 focus:ring-ring/30">
      ...
      {showAdd && <AddDialog groups={groups} onClose={() => setShowAdd(false)} onCreated={() => { setShowAdd(false); setRefreshKey((k) => k + 1) }} />}
      {confirmDelete && <ConfirmDialog title="Delete modifier?" description={`Removes "${confirmDelete.name}".`} onConfirm={() => handleDelete(confirmDelete)} onCancel={() => setConfirmDelete(null)} />}
    </div>
  )
}

function AddDialog({ groups, onClose, onCreated }: { groups: ModifierGroupRow[]; onClose: () => void; onCreated: () => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setErrors({})
    try {
      await api.post("/api/modifiers", { modifierGroupId, name, priceDelta: Number(priceDelta) })
  ...
  return (
    <Modal title="New modifier" onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-4">
        <Field label="Modifier group" error={errors.modifiergroupid}>
          <select value={modifierGroupId} onChange={(e) => setModifierGroupId(e.target.value)} required className="h-9 w-full rounded-md border bg-transparent px-3 text-sm outline-none focus:border-ring focus:ring-2 focus:ring-ring/30">
            <option value="">Pick a group…</option>
            {groups.map((g) => <option key={g.id} value={g.id}>{g.name}</option>)}
          </select>
        </Field>
        <Field label="Name" error={errors.name}>
          <input value={name} onChange={(e) => setName(e.target.value)} required placeholder="Oat milk" className="h-9 w-full rounded-md border bg-transparent px-3 text-sm outline-none focus:border-ring focus:ring-2 focus:ring-ring/30" />
        </Field>
        ...
        <div className="flex justify-end gap-2">
          ...
          <button type="submit" disabled={submitting} className="h-9 rounded-md bg-primary px-3 text-sm font-medium text-primary-foreground hover:opacity-90 disabled:opacity-50">{submitting ? "Saving…" : "Create modifier"}</button>
        </div>
      </form>
    </Modal>
  )
