...
export default function ModifierGroupsListPage() {
  ...
  const [data, setData] = useState<PagedResult<ModifierGroupItem> | null>(null)
  const [loading, setLoading] = useState(true)
  const [refreshKey, setRefreshKey] = useState(0)
  const [showAdd, setShowAdd] = useState(false)
  const [confirmDelete, setConfirmDelete] = useState<ModifierGroupItem | null>(null)

  useEffect(() => {
    setLoading(true)
    const params = new URLSearchParams({ page: String(page), pageSize: String(PAGE_SIZE), sort: "name" })
    if (filter) params.set("name", filter)
    api.get<PagedResult<ModifierGroupItem>>(`/api/modifier-groups?${params}`)
      .then(setData)
      .catch((err) => { if (err instanceof ApiError) toast.error(err.message) })
      .finally(() => setLoading(false))
  }, [page, filter, refreshKey])

  async function handleDelete(item: ModifierGroupItem) {
    try {
      await api.delete(`/api/modifier-groups/${item.id}`)
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <button onClick={() => setShowAdd(true)} className="inline-flex h-9 items-center gap-2 rounded-md bg-primary px-3 text-sm font-medium text-primary-foreground hover:opacity-90">
          <Plus className="h-4 w-4" />New modifier group
        </button>
      </div>
      ...
      {confirmDelete && <ConfirmDialog title="Delete modifier group?" description={`Removes "${confirmDelete.name}". Modifiers in this group must be moved or deleted first.`} onConfirm={() => handleDelete(confirmDelete)} onCancel={() => setConfirmDelete(null)} />}
    </div>
  )
}
...
function AddDialog({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setErrors({})
    try {
      await api.post("/api/modifier-groups", { name })
    ...
    <Modal title="New modifier group" onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex justify-end gap-2">
          ...
          <button type="submit" disabled={submitting} className="h-9 rounded-md bg-primary px-3 text-sm font-medium text-primary-foreground hover:opacity-90 disabled:opacity-50">{submitting ? "Saving…" : "Create modifier group"}</button>
        </div>
      </form>
    </Modal>
  )
