...
export default function EditModifierGroupPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params)
  ...
  useEffect(() => {
    api.get<ModifierGroupDetail>(`/api/modifier-groups/${id}`)
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setErrors({})
    try {
      await api.put(`/api/modifier-groups/${id}`, { name })
