...
interface SalesChannelItem { id: string; name: string; isTakeout: boolean; createdAt: string }
...
export default function SalesChannelsListPage() {
  ...
  const [data, setData] = useState<PagedResult<SalesChannelItem> | null>(null)
  const [loading, setLoading] = useState(true)
  const [refreshKey, setRefreshKey] = useState(0)
  const [showAdd, setShowAdd] = useState(false)
  const [confirmDelete, setConfirmDelete] = useState<SalesChannelItem | null>(null)

  useEffect(() => {
    setLoading(true)
    const params = new URLSearchParams({ page: String(page), pageSize: String(PAGE_SIZE), sort: "name" })
    if (filter) params.set("name", filter)
    api.get<PagedResult<SalesChannelItem>>(`/api/sales-channels?${params}`)
      .then(setData)
      .catch((err) => { if (err instanceof ApiError) toast.error(err.message) })
      .finally(() => setLoading(false))
  }, [page, filter, refreshKey])

  async function handleDelete(item: SalesChannelItem) {
    try {
      await api.delete(`/api/sales-channels/${item.id}`)
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <button onClick={() => setShowAdd(true)} className="inline-flex h-9 items-center gap-2 rounded-md bg-primary px-3 text-sm font-medium text-primary-foreground hover:opacity-90">
          <Plus className="h-4 w-4" />New sales channel
        </button>
      </div>
            ...
            {!loading && data?.items.map((it) => (
              <tr key={it.id} className="border-t">
                ...
                <td className="px-3 py-2">
                  <span className={"inline-flex rounded-full px-2.5 py-0.5 text-xs " + (it.isTakeout ? "bg-primary/10 text-primary" : "bg-secondary text-secondary-foreground")}>
                    {it.isTakeout ? "Takeout" : "Dine-in"}
                  </span>
                </td>
                <td className="px-3 py-2 text-muted-foreground">{new Date(it.createdAt).toLocaleDateString()}</td>
                <td className="px-3 py-2">
                  <div className="flex justify-end gap-1">
                    <Link href={`/admin/sales-channels/${it.id}`} className="inline-flex h-8 w-8 items-center justify-center rounded-md hover:bg-accent" aria-label="Edit"><Pencil className="h-4 w-4" /></Link>
      ...
      {confirmDelete && <ConfirmDialog title="Delete sales channel?" description={`Removes "${confirmDelete.name}". Past orders keep their channel reference.`} onConfirm={() => handleDelete(confirmDelete)} onCancel={() => setConfirmDelete(null)} />}
    </div>
  )
}
...
function AddDialog({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setErrors({})
    try {
      await api.post("/api/sales-channels", { name, isTakeout })
  ...
  return (
    <Modal title="New sales channel" onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-4">
        <Field label="Name" error={errors.name}>
          <input value={name} onChange={(e) => setName(e.target.value)} required placeholder="Dine-in / Takeout / Delivery" className="h-9 w-full rounded-md border bg-transparent px-3 text-sm outline-none focus:border-ring focus:ring-2 focus:ring-ring/30" />
        </Field>
        ...
        <div className="flex justify-end gap-2">
          ...
          <button type="submit" disabled={submitting} className="h-9 rounded-md bg-primary px-3 text-sm font-medium text-primary-foreground hover:opacity-90 disabled:opacity-50">{submitting ? "Saving…" : "Create sales channel"}</button>
        </div>
      </form>
    </Modal>
  )
