...
async function createSalesChannel(formData: FormData) {
  "use server"
  await api.post("/api/sales-channels", {
    name: formData.get("name") as string,
    isTakeout: formData.get("isTakeout") === "on",
  })
  revalidatePath("/admin/sales-channels")
}
...
export default async function SalesChannelsListPage({ searchParams }: { searchParams: Promise<{ page?: string; q?: string; sort?: string }> }) {
  ...
  const data = await api.get<PagedResult<SalesChannelItem>>(`/api/sales-channels?${search}`)
    ...
    <div className="space-y-6">
      ...
      <form action={createSalesChannel} className="flex flex-wrap items-end gap-3 rounded-md border bg-card p-3">
        ...
        <label className="flex h-9 cursor-pointer items-center gap-2 text-sm">
          ...
          <span>Takeout (off-premises)</span>
        </label>
            ...
            {data.items.length === 0 && (
              <tr><td colSpan={4} className="px-3 py-12">
                <div className="flex flex-col items-center gap-3 text-muted-foreground">
                  ...
                  <p>No sales channels match this filter.</p>
                </div>
              </td></tr>
            )}
            {data.items.map((it) => (
              <tr key={it.id} className="border-t">
                ...
                <td className="px-3 py-2">
                  <div className="flex justify-end gap-1">
                    ...
                    <form action={deleteSalesChannel.bind(null, it.id)}>
