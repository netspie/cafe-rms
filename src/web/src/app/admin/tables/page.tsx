...
async function createTable(formData: FormData) {
  "use server"
  await api.post("/api/tables", { name: formData.get("name") as string })
...
export default async function TablesListPage({ searchParams }: { searchParams: Promise<{ page?: string; q?: string; sort?: string }> }) {
  ...
  const data = await api.get<PagedResult<TableItem>>(`/api/tables?${search}`)
  ...
  return (
    <div className="space-y-6">
      ...
      <form action={createTable} className="flex items-end gap-3 rounded-md border bg-card p-3">
        <Field label="Name">
          <input name="name" required placeholder="New table" className="h-9 w-48 rounded-md border bg-transparent px-3 text-sm outline-none focus:border-ring focus:ring-2 focus:ring-ring/30" />
        </Field>
              ...
              <tr key={it.id} className="border-t">
                ...
                <td className="px-3 py-2">
                  <div className="flex justify-end gap-1">
                    ...
                    <form action={deleteTable.bind(null, it.id)}>
