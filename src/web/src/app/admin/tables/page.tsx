...
interface TableItem { id: string; name: string; createdAt: string }
...
export default function TablesListPage() {
  ...
  return (
    <div className="space-y-6">
              ...
              <tr key={it.id} className="border-t">
                ...
                <td className="px-3 py-2">
                  <div className="flex justify-end gap-1">
                    <Link href={`/admin/tables/${it.id}`} className="inline-flex h-8 w-8 items-center justify-center rounded-md hover:bg-accent" aria-label="Edit"><Pencil className="h-4 w-4" /></Link>
      ...
      {confirmDelete && <ConfirmDialog title="Delete table?" description={`Removes "${confirmDelete.name}".`} onConfirm={() => handleDelete(confirmDelete)} onCancel={() => setConfirmDelete(null)} />}
    </div>
  )
}
    ...
    <Modal title="New table" onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex justify-end gap-2">
          ...
          <button type="submit" disabled={submitting} className="h-9 rounded-md bg-primary px-3 text-sm font-medium text-primary-foreground hover:opacity-90 disabled:opacity-50">{submitting ? "Saving…" : "Create table"}</button>
        </div>
      </form>
    </Modal>
  )
