...
export default function PrintoutTemplatesListPage() {
  ...
  const [data, setData] = useState<PagedResult<TemplateItem> | null>(null)
  ...
  useEffect(() => {
    ...
    api.get<PagedResult<TemplateItem>>(`/api/printout-templates?${params}`)
      .then(setData)
  ...
  async function handleDelete(item: TemplateItem) {
    try {
      await api.delete(`/api/printout-templates/${item.id}`)
  ...
  return (
    <div className="space-y-6">
      ...
      {confirmDelete && <ConfirmDialog title="Delete template?" description={`Removes "${confirmDelete.name}".`} onConfirm={() => handleDelete(confirmDelete)} onCancel={() => setConfirmDelete(null)} />}
    </div>
  )
}
...
function AddDialog({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setErrors({})
    try {
      await api.post("/api/printout-templates", { name, templateFileUrl })
  ...
  return (
    <Modal title="New template" onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <Field label="Template file URL" hint="Points at a Word document staff can swap out." error={errors.templatefileurl}>
        ...
        <div className="flex justify-end gap-2">
          ...
          <button type="submit" disabled={submitting} className="h-9 rounded-md bg-primary px-3 text-sm font-medium text-primary-foreground hover:opacity-90 disabled:opacity-50">{submitting ? "Saving…" : "Create template"}</button>
        </div>
      </form>
    </Modal>
  )
