...
interface TemplateItem { id: string; name: string; templateFileUrl: string; createdAt: string }

const PAGE_SIZE = 20

async function createTemplate(formData: FormData) {
  "use server"
  await api.post("/api/printout-templates", {
...
async function deleteTemplate(id: string) {
  "use server"
  await api.delete(`/api/printout-templates/${id}`)
  revalidatePath("/admin/printout-templates")
}

export default async function PrintoutTemplatesListPage({ searchParams }: { searchParams: Promise<{ page?: string; q?: string; sort?: string }> }) {
  ...
  const data = await api.get<PagedResult<TemplateItem>>(`/api/printout-templates?${search}`)
  ...
  return (
    <div className="space-y-6">
      ...
      <form action={createTemplate} className="flex items-end gap-3 rounded-md border bg-card p-3">
        <Field label="Name">
          <input name="name" required placeholder="Receipt" className="h-9 w-48 rounded-md border bg-transparent px-3 text-sm outline-none focus:border-ring focus:ring-2 focus:ring-ring/30" />
        </Field>
        <Field label="Template file URL">
          <input name="templateFileUrl" required placeholder="https://… (.docx)" className="h-9 w-72 rounded-md border bg-transparent px-3 text-sm outline-none focus:border-ring focus:ring-2 focus:ring-ring/30" />
        </Field>
            ...
            {data.items.length === 0 && (
              <tr><td colSpan={4} className="px-3 py-12">
                <div className="flex flex-col items-center gap-3 text-muted-foreground">
                  ...
                  <p>No printout templates match this filter.</p>
                </div>
              </td></tr>
            )}
            {data.items.map((it) => (
              <tr key={it.id} className="border-t">
                ...
                <td className="px-3 py-2 font-mono text-xs text-muted-foreground truncate max-w-md">{it.templateFileUrl}</td>
                <td className="px-3 py-2 text-muted-foreground">{new Date(it.createdAt).toLocaleDateString()}</td>
                <td className="px-3 py-2">
                  <div className="flex justify-end gap-1">
                    <Link href={`/admin/printout-templates/${it.id}`} className="inline-flex h-8 w-8 items-center justify-center rounded-md hover:bg-accent" aria-label="Edit"><Pencil className="h-4 w-4" /></Link>
                    <form action={deleteTemplate.bind(null, it.id)}>
