...
- [x] `shadcn init` + baseline components — settled on the **base-nova** style (default in current shadcn CLI), which uses `@base-ui/react` underneath instead of Radix. **Important difference:** components use a `render` prop (e.g. `<Button render={<Link href="…" />}>`) instead of `asChild`. Same composability, different API. **Gotcha:** when rendering a plain `<Button>` as something other than a `<button>` (e.g. via `render={<Link …/>}`), you must also pass `nativeButton={false}` — base-ui defaults to `nativeButton: true` and warns at runtime if the resolved element isn't `<button>`. Sidebar/DropdownMenu primitives don't share this default; only the plain `Button`.
